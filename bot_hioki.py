...
5. HARMONIC EXTRACTION: Look for the 'Level Harmonik' column. Extract ONLY the absolute Current value (in Amperes). You MUST IGNORE the 'Kandungan Harmonik' column (the percentage values like '100.00%').
6. JSON FORMATTING: All measurement values MUST be returned as raw numerical JSON data types (e.g., 227.4). Do NOT wrap measurement numbers in quotes. If a value is missing, return null.
